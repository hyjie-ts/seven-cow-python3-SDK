...
def signing(secret_key, data):
    ...
    # print(type(data))
    # print(bytes(secret_key,encoding="utf-8"))
    

    return urlsafe_b64encode(
        hmac.new(bytes(secret_key,encoding="utf-8"), data, hashlib.sha1).digest()
    )
...
class UploadToken(object):
    ...
    def _make_token(self):
        ...
        info = urlsafe_b64encode(bytes(json.dumps(info),encoding="utf-8"))
...
class Cow(object):
    ...
    def generate_upload_token(self, scope, ttl=3600):
        ...
        # print(self.upload_tokens[scope].token)
    ...
    def put(self, scope, filename, data=None, keep_name=False, override=True):
        ...
        action = '/rs-put/%s' % urlsafe_b64encode(
            bytes('%s:%s' % (scope, upload_name),encoding="utf-8")
        )
        _type, _encoding = mimetypes.guess_type(filename)
        if _type:
            action += '/mimeType/%s' % urlsafe_b64encode(bytes(_type,encoding="utf-8"))
        token = token.replace("b'","")
        token = token.replace("'","")
        action = action.replace("b'","")
